...
void LLCrashLogger::gatherFiles()
{
	...
	*/
	...
	{		
		...
		mFileMap["RainbowViewerLog"] = mDebugLog["SLLog"].asString();
		...
		llinfos << "Using log file from debug log " << mFileMap["RainbowViewerLog"] << llendl;
		llinfos << "Using settings file from debug log " << mFileMap["SettingsXml"] << llendl;
	}
	else
	{
		// Figure out the filename of the rainbow viewer log
		LLCurl::setCAFile(gDirUtilp->getCAFile());
		mFileMap["RainbowViewerLog"] = gDirUtilp->getExpandedFilename(LL_PATH_LOGS,"RainbowViewer.log");
		mFileMap["SettingsXml"] = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS,"settings_1.22.xml");
	}
	...
	if(mCrashInPreviousExec)
	{
		...
		// RainbowViewer.log
		std::string log_filename = mFileMap["RainbowViewerLog"];
		log_filename.replace(log_filename.size() - 4, 4, ".old");
		mFileMap["RainbowViewerLog"] = log_filename;
	}
...
bool LLCrashLogger::init()
{
	...
	// Default to the product name "Rainbow Viewer" (this is overridden by the -name argument)
	mProductName = "Rainbow Viewer";
	...
	if( gDirUtilp )
	{
		std::string marker_file = gDirUtilp->getExpandedFilename(LL_PATH_LOGS,"RainbowViewer.exec_marker");
