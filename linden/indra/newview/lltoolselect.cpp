...
//MK
	...
	if (gRRenabled && object)
	{
		...
		if (!gAgent.mRRInterface.canTouchFar(object, mPick.mIntersection))
		{
			return FALSE;
		}
	}
...
//MK
	if (gRRenabled && object)
	{
		if (!temp_select && !gAgent.mRRInterface.canEdit(object))
		{
			return FALSE;
		}
		if (!gAgent.mRRInterface.canTouchFar(object))
		{
			return FALSE;
		}
	}
