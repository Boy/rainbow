...
U32 LLVOAvatar::renderSkinned(EAvatarRenderPass pass)
{
	...
	if (pass == AVATAR_RENDER_PASS_SINGLE)
	{
		...
		if (!mIsSelf || gAgent.needsRenderHead())
		{
			...
			{
				num_indices += mMeshLOD[MESH_ID_HEAD]->render(mAdjustedPixelArea, first_pass);
				first_pass = FALSE;
			}
		}
...
U32 LLVOAvatar::renderRigid()
{
	...
	{
		num_indices += mMeshLOD[MESH_ID_EYEBALL_LEFT]->render(mAdjustedPixelArea, TRUE);
		num_indices += mMeshLOD[MESH_ID_EYEBALL_RIGHT]->render(mAdjustedPixelArea, TRUE);
	}
...
{	
	...
	{
		const LLVOAvatarDictionary::TextureDictionaryEntry *text_dict = iter->second;
		LLViewerImage* te_image = getTEImage(iter->first);
		if( !te_image )
		{
		...
		if(aux_src && aux_src->getComponents() == 1)
		{
				 ...
				 iter++)
			{
				...
				if (text_dict->mIsUsedByBakedTexture)
				{
					...
					LLViewerImage *baked_img = self->getTEImage(texture_index);
					if (baked_img && id == baked_img->getID())
					{
						...
						if (self->mBakedTextureData[baked_index].mTexLayerSet)
						{
							...
							{
								glDeleteTextures(1, (GLuint*) &(self->mBakedTextureData[baked_index].mMaskTexName));
							}
...
void LLVOAvatar::dumpLocalTextures()
{
		 ...
		 iter++)
	{
		...
		const std::string &name = text_dict->mName;
		LocalTextureData &local_tex_data = mLocalTextureData[iter->first];
		...
		else if (local_tex_data.mImage.notNull())
		{
			...
			{
				LLViewerImage* image = local_tex_data.mImage;
