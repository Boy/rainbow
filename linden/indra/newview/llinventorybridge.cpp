...
void rez_attachment(LLViewerInventoryItem* item, LLViewerJointAttachment* attachment, bool replace)
{
	...
	rez_action->mItemID = item->getLinkedUUID(); // Wear the base object in case this is a link.
	LLVOAvatar* avatar = gAgent.getAvatarObject();
	S32 attach_pt = 0;
	...
	if (!replace && avatar && avatar->allowMultipleAttachments())
	{
	...
	{
//MK
		if (!gRRenabled || (gAgent.mRRInterface.canAttach(item) && gAgent.mRRInterface.canDetach(attachment->getName())))
		{
//mk
...
void confirm_replace_attachment_rez(S32 option, void* user_data)
{
	...
	if (!gAgent.getAvatarObject()->canAttachMoreObjects())
	{
		...
		gViewerWindow->alertXml("MaxAttachmentsOnOutfit", args);
		return;
	}
...
//MK
				if (gRRenabled && !gAgent.mRRInterface.canDetach(avatarp->getWornAttachment(mUUID), FALSE))
				{
		...
		if( obj_count > 0 )
		{
			...
			if( avatar )
			{
				...
				for(i = 0; i < obj_count; ++i)
				{
					...
					bool replace = wear_info->mReplace || !avatar->allowMultipleAttachments();
...
void remove_inventory_category_from_avatar_step2( BOOL proceed, void* userdata)
{
	...
	if(proceed)
	{
		...
		if (obj_count > 0)
		{
			...
			{
//MK
				if (gRRenabled && !gAgent.mRRInterface.canDetach(obj_item_array.get(i)))
				{
					continue;
				}
//mk
				...
				gMessageSystem->nextBlockFast(_PREHASH_ObjectData );
				gMessageSystem->addUUIDFast(_PREHASH_AgentID, gAgent.getID() );
				gMessageSystem->addUUIDFast(_PREHASH_ItemID, obj_item_array.get(i)->getLinkedUUID());

				gMessageSystem->sendReliable( gAgent.getRegion()->getHost() );

				// this object might have been selected, so let the selection manager know it's gone now
				LLViewerObject *found_obj = gObjectList.findObject( obj_item_array.get(i)->getLinkedUUID());
				if (found_obj)
				{
					LLSelectMgr::getInstance()->remove(found_obj);
				}
...
//MK
		if (gRRenabled && item)
		{
			...
			LLInventoryItem *old_item = gAgent.getWearableInventoryItem(type);
			if (old_item)
			{
				LLViewerInventoryItem* vitem = dynamic_cast<LLViewerInventoryItem*>(old_item);
				if (vitem && !gAgent.mRRInterface.canUnwear(vitem))
				{
					// cannot remove the old item, so cannot replace it either
					return;
				}
			}
